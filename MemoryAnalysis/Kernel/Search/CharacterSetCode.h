...
#include <string>
using namespace std;
class CCharacterSetCode
{
public:
	CCharacterSetCode(void);
	~CCharacterSetCode(void);
protected:
	static void transfer(CString before,wstring& after);
public:
	static void UTF_8ToUnicode(wchar_t* pOut,char *pText);  // 把UTF-8转换成Unicode——成功的
	static string UTF_8toString(CString utf8);
